<template>
  <div class="starry-sky">
    <!-- 卡片列表展示论文和相关笔记信息 -->
    <n-list :show-divider="false" class="ap-large-list starry-sky__paper-list">
      <template #header>
        文献
      </template>
      ...
      <n-card hoverable class="ap-large-list" v-for="(paper, i) in paperCardInfoList">
        <n-list-item>
          <template #prefix>
            <n-image
              width="200"
          ...
          <template #suffix>
            <n-button  @click="showIntroDrawer = !showIntroDrawer">
              介绍
            </n-button>
          </template>
            {{ paper.name }}
    ...
    <!-- 论文简介（作者、发表期刊、年限、摘要、被引用次数、笔记数） + 评论 + 前几笔记介绍列表 -->
    <n-drawer
      v-model:show="showIntroDrawer"
      :default-width="350"
      resizable
      :show-mask="false"
    >
    <!-- TODO 这里直接paper.name会有问题 -->
      <n-drawer-content title="paper.name" :native-scrollbar="false">
